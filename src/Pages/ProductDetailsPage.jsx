...
import { reviewApi, isMongoId } from "../services/api";
      ...
      <section className="mx-auto w-[min(1280px,calc(100%-80px))] max-sm:w-[calc(100%-32px)]">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_630px]">
          ...
          <div className="grid grid-cols-3 gap-6">
            {(product.gallery ?? [product.image, product.image, product.image]).map(
              (img, i) => (
                <button
                  key={i}
                  type="button"
        ...
        <div className="mt-8 flex flex-wrap items-end gap-x-10 gap-y-2 border-b border-slate-300 pb-3">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Review <span className="text-[#27489f]">({product.rating})</span>
            </p>
          </div>
        ...
        <section className="border-b border-slate-300 py-3">
          ...
          <p className="mt-2 text-lg text-slate-700">{product.description}</p>
        </section>
              ...
              {product.sizes.map((s) => (
                <button
                  ...
                  className={`grid h-10 w-10 place-items-center rounded-full font-bold ${
                    size === s ? "bg-[#27489f] text-white" : "bg-[#e8eeff] text-[#27489f]"
                  }`}
                >
                  {s}
                </button>
        ...
        <section className="mt-10 border-t border-slate-200 pt-8">
          ...
          {/* ── Leave a review form ─────────────────────── */}
          <form onSubmit={submitReview} className="mt-8 rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
            <h3 className="text-xl font-semibold">Leave a Review</h3>

            <div className="mt-4">
              <label className="mb-1 block text-sm font-medium text-slate-600">Rating</label>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    ...
                    className={`text-2xl ${
                      n <= reviewForm.rating ? "text-[#f4c542]" : "text-slate-300"
                    }`}
                  >
                    ★
                  </button>
            ...
            {reviewError && (
              <p className="mt-2 text-sm font-semibold text-red-600">{reviewError}</p>
            )}
            {reviewSuccess && (
